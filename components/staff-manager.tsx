...
const STATUS_STYLES: Record<string, string> = {
  ...
  inactive: "bg-[#1E2640] text-[#64748B] border-[#252B45]",
  on_leave: "bg-[#78350F]/30 text-[#F59E0B] border-[#92400E]",
};
    ...
    <>
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div className="flex gap-2 flex-wrap">
          <button onClick={() => setFilter("all")}
            className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${clubFilter === "all" ? "bg-[#3B1F7A] text-[#A78BFA]" : "bg-[#131729] border border-[#252B45] text-[#64748B] hover:text-[#94A3B8]"}`}>
          ...
          {clubs.map((c) => {
            ...
            return (
              <button key={c.id} onClick={() => setFilter(c.id)}
                className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${clubFilter === c.id ? "bg-[#3B1F7A] text-[#A78BFA]" : "bg-[#131729] border border-[#252B45] text-[#64748B] hover:text-[#94A3B8]"}`}>
      ...
      {filtered.length === 0 ? (
        <div className="bg-[#131729] border border-[#252B45] rounded-xl p-16 flex flex-col items-center justify-center text-center">
          ...
          <h2 className="text-lg font-bold text-[#F1F5F9] mb-2">No staff yet</h2>
          <p className="text-[#64748B] text-sm mb-6 max-w-sm">Add staff members to track positions, contact details, and status.</p>
          <button onClick={openCreate}
        ...
        <div className="bg-[#131729] border border-[#252B45] rounded-xl overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.map((m) => (
                  <tr key={m.id} className="border-t border-[#252B45]/60 hover:bg-[#1A1F35]/50 transition-colors">
                    <td className="px-4 py-3 font-semibold text-[#F1F5F9]">{m.name}</td>
                    <td className="px-4 py-3 text-[#94A3B8]">{m.position ?? "—"}</td>
                    <td className="px-4 py-3 text-[#94A3B8]">{m.club?.name ?? "—"}</td>
                    <td className="px-4 py-3">
                      <div className="flex flex-col gap-0.5">
                        {m.email && <span className="flex items-center gap-1 text-[#64748B] text-xs"><Mail size={10} />{m.email}</span>}
                        {m.phone && <span className="flex items-center gap-1 text-[#64748B] text-xs"><Phone size={10} />{m.phone}</span>}
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <button onClick={() => openEdit(m)} className="p-1.5 rounded text-[#64748B] hover:text-[#A78BFA] hover:bg-[#1A1F35] transition-colors"><Pencil size={13} /></button>
                        <button onClick={() => del(m.id)} disabled={deleting === m.id} className="p-1.5 rounded text-[#64748B] hover:text-[#EF4444] hover:bg-[#7F1D1D]/20 transition-colors disabled:opacity-40">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-[#131729] border border-[#252B45] rounded-2xl w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#252B45]">
              <h2 className="text-lg font-bold text-[#F1F5F9]">{modal === "create" ? "Add Staff Member" : "Edit Staff Member"}</h2>
            ...
            <div className="px-6 py-5 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <label className="block text-xs font-semibold text-[#94A3B8] uppercase tracking-wide mb-1.5">Full Name <span className="text-[#EF4444]">*</span></label>
                ...
                <div>
                  <label className="block text-xs font-semibold text-[#94A3B8] uppercase tracking-wide mb-1.5">Position</label>
                  <input value={form.position} onChange={(e) => f("position", e.target.value)} placeholder="Club Manager"
                    className="w-full px-3 py-2 bg-[#0B0E1A] border border-[#252B45] rounded-lg text-[#F1F5F9] text-sm placeholder-[#475569] focus:outline-none focus:border-[#7C3AED] transition-colors" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-[#94A3B8] uppercase tracking-wide mb-1.5">Club <span className="text-[#EF4444]">*</span></label>
                ...
                <div>
                  <label className="block text-xs font-semibold text-[#94A3B8] uppercase tracking-wide mb-1.5">Email</label>
                  <input type="email" value={form.email} onChange={(e) => f("email", e.target.value)} placeholder="jane@example.com"
                    className="w-full px-3 py-2 bg-[#0B0E1A] border border-[#252B45] rounded-lg text-[#F1F5F9] text-sm placeholder-[#475569] focus:outline-none focus:border-[#7C3AED] transition-colors" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-[#94A3B8] uppercase tracking-wide mb-1.5">Phone</label>
