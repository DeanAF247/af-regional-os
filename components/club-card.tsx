...
function statusDot(pct: number | null) {
  if (pct == null) return "bg-[#64748B]";
  if (pct >= 90) return "bg-[#10B981] shadow-[0_0_6px_#10B981]";
  if (pct >= 70) return "bg-[#F59E0B] shadow-[0_0_6px_#F59E0B]";
...
function MetricRow({
  ...
  return (
    <div className="mb-3 last:mb-0">
      ...
      <div className="text-[11px] text-[#64748B] mt-0.5 text-right">
...
export default function ClubCard({ club }: { club: ClubKpiData }) {
  ...
  return (
    <Link href={`/clubs/${club.slug}`} className="block group">
      <div className="bg-[#131729] border border-[#252B45] rounded-xl overflow-hidden hover:border-[#7C3AED]/60 transition-all duration-200 hover:shadow-[0_0_20px_#7C3AED18]">
        {/* Header */}
        <div className="bg-[#1A1F35] px-4 py-3 border-b border-[#252B45] flex items-center justify-between">
          <span className="font-bold text-[15px] text-[#F1F5F9] group-hover:text-[#A78BFA] transition-colors">
            {club.name}
          </span>
        ...
        <div className="p-4">
          ...
          {/* Stats row */}
          <div className="grid grid-cols-2 gap-3 mt-3 pt-3 border-t border-[#252B45]">
            <div>
              <div className="text-[10px] text-[#64748B] uppercase tracking-wide mb-0.5">NNM</div>
              <div className={cn(
                "text-lg font-extrabold",
                club.nnm_actual != null && club.nnm_target != null
                  ? club.nnm_actual >= club.nnm_target ? "text-[#10B981]" : "text-[#EF4444]"
                  : "text-[#94A3B8]"
              )}>
            ...
            <div>
              <div className="text-[10px] text-[#64748B] uppercase tracking-wide mb-0.5">CPL</div>
              <div className="text-lg font-extrabold text-[#A78BFA]">
                {club.cpl != null ? formatCurrency(club.cpl) : "—"}
              </div>
            </div>
            <div>
              <div className="text-[10px] text-[#64748B] uppercase tracking-wide mb-0.5">Spend</div>
              <div className={cn(
                "text-lg font-extrabold",
                club.spend_pct != null && club.spend_pct > 105 ? "text-[#EF4444]" : "text-[#10B981]"
              )}>
                {club.spend_actual != null ? formatCurrency(club.spend_actual) : "—"}
              </div>
            </div>
            <div>
              <div className="text-[10px] text-[#64748B] uppercase tracking-wide mb-0.5">Budget</div>
              <div className="text-lg font-extrabold text-[#94A3B8]">
